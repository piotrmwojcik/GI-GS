...
def eval_brdf(data_root: str, scene: Scene, model_path: str, name: str) -> None:
    ...
    for idx, frame in enumerate(tqdm(frames)):
        ...
        if "orb" in data_root:
            ...
            albedo_gt = np.array(Image.open(os.path.join(data_root, 'golden_bay_4k_32x16_rot330', albedo_path)))[..., :3]
        ...
        if "orb" in data_root:
            ...
            mask = np.array(Image.open(os.path.join(data_root, 'golden_bay_4k_32x16_rot330', albedo_path)))[..., 3] > 0
            expanded_mask = np.expand_dims(mask, axis=-1)
            mask_3d = np.repeat(expanded_mask, 3, axis=-1)
    ...
    for idx, (mask, albedo_map, albedo_gt) in enumerate(tqdm(zip(masks, albedo_maps, albedo_gts))):
        ...
        three_channel_ratio = (albedo_map / albedo_ours.clamp_min(1e-6)).median(dim=0).values.tolist()
        print('!! mask')
